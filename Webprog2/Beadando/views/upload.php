...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    if(count($errors) == 0)
    {
        $sql = $db->prepare("INSERT INTO `comicbooks` (`title`, `writer`, `illustrator`, `year`, `cover`, `desc`, `userId`) VALUES (?,?,?,?,?,?,?)");
        $sql->bind_param("sssissi", $title, $artist_w, $artist_i, $year, $fullpath, $desc,$_SESSION['user_userId']);
        $sql->execute();
        $sql->close();
    }
}
